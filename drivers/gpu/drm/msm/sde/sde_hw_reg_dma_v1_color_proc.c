...
void reg_dmav1_setup_dspp_igcv31(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	if (!hw_cfg->payload) {
		...
		printk("[Display] igc has been disabled\n");
	...
	if (asus_igc_need_commit) {
		printk("[Display] first igc commit, apply igc\n");
		asus_igc_need_commit = false;
	} else if (!asus_igc_enabled) {
		printk("[Display] re-enabled igc, apply igc\n");
		asus_igc_enabled = true;
	} else if (asus_igc_lut_table_compare(lut_cfg)) {
		printk("[Display] igc lut #### SAME ####  \n");
		return;
	} else {
		printk("[Display] igc lut #### DIFF ####  \n");
	}
#endif
