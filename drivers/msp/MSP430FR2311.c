...
//		ret = MSP430FR2311_power_control(0);
	...
	if (g_ASUS_hwID == HW_REV_ER	|| g_ASUS_hwID == HW_REV_SR   || g_ASUS_hwID == HW_REV_PR) {
		fManualMode=ManualMode_BeforePR;
		D("[MCU][MSP430FR2311] %s: Probe success, manual mode=ER!\n", __func__);
	} else {
		fManualMode=ManualMode_AfterAndPR2;
		D("[MCU][MSP430FR2311] %s: Probe success, manual mode=PR2!\n", __func__);

		
//		fManualMode=FrqConvertMode;
//        D("[MCU][MSP430FR2311] %s: Probe success, DEBUG !!!!! manual mode=FrqConvertMode!, remove in shipping version", __func__);
	}
