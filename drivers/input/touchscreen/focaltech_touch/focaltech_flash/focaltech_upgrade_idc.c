...
int fts_ctpm_upgrade_idc_init(struct i2c_client * client, u32 dw_lenth)
{
   ...
   FTS_FUNC_ENTER();
    ...
    FTS_DEBUG( "[UPGRADE]: flash id : 0x%x!!", reg_val_id[0]);
...
/************************************************************************
* Name: fts_ctpm_check_in_pramboot
* Brief: 0
* Input: 0
* Output: 0
* Return: 0
...
bool fts_ctpm_check_in_pramboot(struct i2c_client * client)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        inRomBoot = fts_ctpm_get_pram_or_rom_id(client);
        printk( "[UPGRADE]: inRomBoot = %d", inRomBoot);
        if (inRomBoot == FTS_RUN_IN_PRAM)
        {
            printk( "[UPGRADE]: run in pram!!");
            return true;
        }
        else
        {
            printk( "[UPGRADE]: not run in pram!!");
            continue;
        }
    }
...
int fts_ctpm_upgrade_ecc(struct i2c_client * client, u32 startaddr, u32 length)
{
    ...
    printk("[fts][touch][UPGRADE]: enter %s:%d\n", __func__, __LINE__);
    printk("[fts][touch][UPGRADE]: read out checksum!!");
    ...
    printk("[fts][touch][UPGRADE]: ecc success : FW=%02x upgrade_ecc=%02x!!", reg_val[0], upgrade_ecc);
...
int fts_ctpm_write_pramboot_for_idc(struct i2c_client * client, u32 length, u8* readbuf)
{
    ...
    u32 temp;
    u32 packet_number;
    u8 *packet_buf = fwupgrade->file_buf;
    ...
    upgrade_ecc = 0;
    printk("[fts][touch][UPGRADE]: enter %s:%d\n", __func__, __LINE__);
    printk("[fts][touch][UPGRADE]: write pramboot to pram!!");

    if (NULL == readbuf) {
        printk("pramboot buf is null");
        return -EINVAL;
    }
    ...
    FTS_FUNC_EXIT();
    printk("[fts][touch][UPGRADE]: %s end !!!\n", __func__);
    return 0;
}
...
int fts_ctpm_write_app_for_idc(struct i2c_client * client, u32 length, u8* readbuf)
{
    ...
    printk("[fts][touch][UPGRADE]: enter %s:%d\n", __func__, __LINE__);
    printk("[fts][touch][UPGRADE]: write app to flash!!");
    upgrade_ecc = 0;
