...
#endif
//#define FTS_READ_TOUCH_BUFFER_DIVIDED wake_lock       0
/*****************************************************************************
...
int fts_wait_tp_to_valid(struct i2c_client *client)
{
    ...
    do
    {
        ...
        {
            pr_err("[FTS_err][touch] TP Not Ready, ReadData = 0x%x \n", reg_value);
        }
        else if (reg_value == chip_types.chip_idh)
        {
            pr_err("[FTS][touch] TP Ready, Device ID = 0x%x", reg_value);
            return 0;
        }
        cnt++;
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    int max_touch_num = FTS_MAX_POINTS;
	...
	pointid = (buf[FTS_TOUCH_ID_POS + FTS_ONE_TCH_LEN * i]) >> 4;
        ...
        if ((event->au8_touch_event[i]==0 || event->au8_touch_event[i]==2)&&(data->point_num==0))
        {
            return -1;
        }
    }
...
{
    int i;
    //printk("[focal]start fts_input_dev_report_key_event \n");
#if FTS_TEST_EN
    ...
    {
        if ( (1 == data->touch_point || 1 == data->point_num) &&
...
static int fts_input_dev_report_b(struct ts_event *event, struct fts_ts_data *data)
{
    ...
    //printk("[FTS][touch]fts_input_dev_report_b\n");
    ...
    else
    {
        ...
        input_report_key(data->input_dev, BTN_TOUCH, data->touch_point > 0);
    }
...
static void fts_report_value(struct fts_ts_data *data)
{
    ...
    //printk("[FTS][touch]fts_report_value\n");
#if FTS_MT_PROTOCOL_B_EN
...
static irqreturn_t fts_ts_interrupt(int irq, void *dev_id)
{
    ...
    //printk("[FTS][touch] fts_ts_interrupt!\n");
    if (!fts_ts)
    {
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
	...
	printk("[FTS] set reset gpio %d init low\n", data->pdata->reset_gpio);
    }

    printk("[FTS] irq pin = %d !!!\n", gpio_get_value(data->pdata->irq_gpio));
    printk("[FTS] reset pin = %d !!!\n", gpio_get_value(data->pdata->reset_gpio));
    printk("[FTS] fts_gpio_configure OK!!");
                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    printk("[FTS][touch] print coords define X: from %d to %d , Y: from %d to %d \n", pdata->x_min, pdata->x_max, pdata->y_min, pdata->y_max);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    printk("[FTS] print gpio info reset pin: %d irq pin %d  \n", pdata->reset_gpio, pdata->irq_gpio);
    FTS_FUNC_EXIT();
    return 0;
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    FTS_FUNC_ENTER();
    printk("[FTS][touch]fts_ts_probe !!!!!\n");
    //printk("[FTS][touch]g_asus_lcdID = %d\n", g_asus_lcdID);
    /* 1. Get Platform data */

    if (client->dev.of_node)
    {
        //printk("[fts][touch]: enter %s:%d\n", __func__, __LINE__);
	pdata = devm_kzalloc(&client->dev, sizeof(*pdata), GFP_KERNEL);
        if (!pdata)
        {
        
            pr_err("[FTS_err][touch][MEMORY]Failed to allocate memory \n");
            pr_err("[FTS][touch] fts_ts_prob_exit \n");
            return -ENOMEM;
        }
    ...
    if (!pdata)
    {
        pr_err("[FTS_err][touch]Invalid pdata \n");
        pr_err("[FTS][touch] fts_ts_prob_exit \n");
        return -EINVAL;
    }
    ...
    {
        pr_err("[FTS_err][touch]I2C not supported\n");
         pr_err("[FTS][touch] fts_ts_prob_exit \n");
        return -ENODEV;
    }
    ...
    if (!data)
    {
        pr_err("[FTS-err][touch][MEMORY]Failed to allocate memory\n");
        pr_err("[FTS][touch] fts_ts_prob_exit \n");
        return -ENOMEM;
    }

    printk("[FTS][touch] fts_input_init start\n");
    input_dev = input_allocate_device();
    if (!input_dev)
    {
        pr_err("[FTS_err][touch][INPUT]Failed to allocate input device \n");
        pr_err("[FTS][touch] fts_ts_prob_exit \n");
        return -ENOMEM;
    }
    ...
    if (data->pdata->have_key)
    {
         pr_err("[FTS][touch]set key capabilities \n");
    ...
    data->point_num = pdata->max_touch_number;
    data->pnt_buf_size = data->point_num * FTS_ONE_TCH_LEN + 3;
    data->point_buf = (u8 *)kzalloc(data->pnt_buf_size, GFP_KERNEL);
    if (!data->point_buf) {
        pr_err("failed to alloc memory for point buf!");
        err = -ENOMEM;
        goto err_point_buf;
    }

    data->event = (struct ts_event *)kzalloc(data->point_num * sizeof(struct ts_event), GFP_KERNEL);
    if (!data->event) {
        pr_err("failed to alloc memory for point event!");
        err = -ENOMEM;
        goto err_event_buf;
    }
    ...
    if (err)
    {
         pr_err("[FTS_err][touch]Input device registration failed \n");
        goto free_inputdev;
    }
    printk("[FTS][touch] fts_input_init end\n");
    ...
    if (err < 0)
    {
         pr_err("[FTS_err][touch][GPIO]Failed to configure the gpios \n");
        goto free_gpio;
    }
    ...
    if (err < 0)
    {
        pr_err("[FTS_err][touch][GPIO]Failed to get TP Ready signal \n");
		goto free_gpio;
    }
    ...
    data->ts_workqueue = create_singlethread_workqueue(FTS_WORKQUEUE_NAME);
    if (!data->ts_workqueue)
    {
        err = -ESRCH;
        pr_err("[FTS_err][touch] Create touch workqueue failed \n");
    ...
    pr_err("[FTS][touch] peint irq num info %d \n", client->irq);
    ...
    if (err)
    {
        pr_err("[FTS_err][touch] Request irq failed! \n");
        goto free_gpio;
    }
    printk("[FTS][touch]reauest irq threaded ok\n");
    ...
    {
        pr_err("[FTS_err][touch] fts_sensor_init failed!");
        FTS_FUNC_EXIT();
        return 0;
    }
#endif
...
static int __init fts_ts_init(void)
{
    ...
    printk(KERN_ERR"[Touch] Focal init start !!!!!\n");

//    FTS_FUNC_ENTER();
    ret = i2c_add_driver(&fts_ts_driver);
    if ( ret != 0 )
    {
        printk(KERN_ERR"[Touch] driver init fail!!!!!\n");
    }
        printk(KERN_ERR"[Touch] Focal init end !!!!!\n");
//    FTS_FUNC_EXIT();
    return ret;
}
