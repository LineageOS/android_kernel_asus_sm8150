...
int fts_getsize(u8 fw_type)
{
    ...
    if (fw_type == FW_SIZE)
    {
	pr_err("[fts][touch] fts_getsize: CTPM_FW_ARA \n");
	//if (g_asus_lcdID == 2)
    ...
    {
        pr_err("[fts][touch] fts_getsize: aucFW_PRAM_BOOT \n");
...
bool fts_ctpm_check_need_upgrade(struct i2c_client *client)
{
    ...
    FTS_FUNC_ENTER();
    printk("[fts][touch] fts_ctpm_check_need_upgrade ! \n");
    for (i=0; i<5; i++)
    {
    ...
    if (fw_status == FTS_RUN_IN_APP) //call fts_flash_get_upgrade_info in probe function firstly.
    {
        ...
        if (uc_tp_fm_ver != uc_host_fm_ver )
        {
            ...
            bUpgradeFlag = true;
            printk("[UPGRADE]: need upgrade!!");
        }
        else
        {
            fts_updatefun_curr.need_upgrade = false;
            printk("[UPGRADE]: no need upgrade!!");
        }

        if (fts_updatefun_curr.get_projectid)
        {
            bUpgradeFlag = true;
            printk("[UPGRADE]: need get project id!!");
        }
    }
    else
    {
        /* if app is invalid, reset to run ROM */
        printk("[UPGRADE]: app is invalid, need upgrade, reset now!!");
...
{
    int i_ret = 0;
    printk("[fts][touch][UPGRADE]: enter %s:%d\n", __func__, __LINE__);
...
static void fts_ctpm_update_work_func(struct work_struct *work)
{
    ...
    printk("[fts][touch] [UPGRADE]: disable irq!!\n");
	...
	{
		printk("[fts][touch] [UPGRADE]: uc_tp_fm_ver == 0xef, upgrade again\n");
		fts_ctpm_auto_upgrade(fts_i2c_client);
	}
    ...
    printk("[fts][touch] [UPGRADE]: enable irq!!\n");
    pr_err("[fts][touch] [UPGRADE]:FTS upgrade exit!!\n");
}
...
void fts_ctpm_upgrade_init(void)
{
    ...
    if (NULL == fwupgrade) {
        printk("malloc memory for upgrade fail");
    }
    ...
    fwupgrade->file_buf = (u8 *)kzalloc((FTS_PACKET_LENGTH + 6), GFP_KERNEL);
    if (!fwupgrade->file_buf) {
