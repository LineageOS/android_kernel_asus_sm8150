...
static ssize_t set_reset_pin_level_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (gpio_get_value(fts_wq_data->pdata->reset_gpio))
		stat = true;
	else
		stat = false;

	return sprintf(buf, "reset_pin_status = %d and realy reset pin level = %d \n", fts_wq_data->pdata->reset_pin_status, stat);
}
...
static ssize_t fts_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (2 == num_read_chars)
    {
        ...
        if (g_rwreg_result.result < 0)
        {
            pr_err("[touch]Could not read the register(0x%02x)\n", regaddr);
        }
        else
        {
            pr_err("[touch]the register(0x%02x) is 0x%02x\n", regaddr, regvalue);
    ...
    else
    {
        ...
        if (g_rwreg_result.result < 0)
        {
            pr_err("[touch]Could not write the register(0x%02x)", regaddr);

        }
        else
        {
            pr_err("[touch]Write 0x%02x into register(0x%02x) successful", regvalue, regaddr);
            g_rwreg_result.result = 0;
        }
    }
#if FTS_ESDCHECK_EN
