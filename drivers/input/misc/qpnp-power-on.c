...
int asus_set_ship_mode_prepare()
{
	regmap_write(gpon->regmap, 0x881, 0x00);
	regmap_write(gpon->regmap, 0x882, 0x00);
	//qpnp_pon_masked_write(gpon, QPNP_PON_PS_HOLD_RST_CTL(gpon),
				   //QPNP_PON_POWER_OFF_MASK, 0x06);
	return 0;
}
 ...
 */
	...
	if (!cfg)
	{
		printk("[keypad] qpnp_get_cfg (%d) fail\n",pon_type);
		return -EINVAL;
	}
	/* Check if key reporting is supported */
	if (!cfg->key_code)
	{
		printk("[keypad] cfg->key_code is null \n");
		return 0;
	}
	...
	if (rc)
	{
		printk("[keypad] qpnp_pon_read fail (%d) \n",rc);
		return rc;
	}
	switch (cfg->pon_type) {
	case PON_KPDPWR:
	...
	pr_debug("[keypad] PMIC input: code=%d, status=0x%02X\n", cfg->key_code,
		pon_rt_sts);
...
static void kpdpwr_reboot_work_func(struct work_struct *work) {
	...
	if(resin_hold_6s_flag) {
		...
		pr_info("%s: pon: hold power key and resin key more than 8s. trigger reboot...\n", __func__);
...
qpnp_pon_request_irqs(struct qpnp_pon *pon, struct qpnp_pon_config *cfg)
{
	...
	if (cfg->key_code) {
		if (cfg->pon_type == PON_RESIN ) {
			...
			//printk("[qpnp] asus_enable_resin_irq_wake(1): test 1\n");
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	gpon = pon;
	//printk("[qpnp] asus_enable_resin_irq_wake(0): test 2\n");
