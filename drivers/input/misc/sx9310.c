...
#define SX9310_NIRQ_ERROR	2
#define SX9310_CONN_ERROR	3
#define SX9310_I2C_ERROR	4
...
//static int sx9310_set_mode(psx93XX_t this, unsigned char mode);
...
static int sx9310_Hardware_Check(psx93XX_t this)
{
	...
	while(this->get_nirq_low && this->get_nirq_low()){
		...
		msleep(100);
		if(++loop > 1){
			this->failStatusCode = SX9310_NIRQ_ERROR;
			break;
		}
	}
	dev_info(this->pdev, "sx9310 failcode - 0 = 0x%x\n",this->failStatusCode);
	
	//Check I2C Connection
	ret = read_register(this, SX9310_WHOAMI_REG, &failcode);
	if(ret < 0){
		this->failStatusCode = SX9310_I2C_ERROR;
	}
	dev_info(this->pdev, "sx9310 failcode - 1 = 0x%x, local failcode = 0x%x\n",this->failStatusCode, failcode);
		
	if(!(failcode == SX9310_WHOAMI_VALUE || failcode == SX9311_WHOAMI_VALUE)){
		pr_info("%s: id error, failcode()", __func__);
		this->failStatusCode = SX9310_ID_ERROR;
	} else
		pr_info("%s: id correct", __func__);
	
	dev_info(this->pdev, "sx9310 failcode - 2 = 0x%x\n",this->failStatusCode);
	return (int)this->failStatusCode;
}


/*********************************************************************/
static int sx9310_global_variable_init(psx93XX_t this)
{
	this->irq_disabled = 0;
	this->failStatusCode = 0;
...
static int read_cap_status(psx93XX_t this)
{
    ...
    for (csx = 0; csx < 2; csx++) {
              write_register(this, SX9310_CPSRD,csx);
              read_register(this, SX9310_USEMSB, &msb);
              read_register(this, SX9310_USELSB, &lsb);
              useful[csx] = (s32)((msb << 8) | lsb);

              if (useful[csx] > 32767)
                  useful[csx] -= 65536;
    }

    if (useful[0] >= 1800 || useful[1] >= 1800) {
        pr_info("[SX9310] Near\n");
    } else {
        pr_info("[SX9310] Far\n");
        return -1;
...
static ssize_t sx9310_cap_status_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
    psx93XX_t this = dev_get_drvdata(dev);
...
{
    u8 reg_val = 0;
    int ret = read_register(this, SX9310_USELSB, &reg_val);
    if (ret < 0) {
        pr_err("[SX9310] read cap2 register fail\n");
        return -1;
    } else {
        if (reg_val == 0xfe) {
            pr_info("[SX9310] CS2: near\n");
        } else {
            pr_info("[SX9310] CS2: far\n");
            return -1;
        }
    }
    return 0;
...
static ssize_t sx9310_cap2_status_show(struct device *dev,
                                                struct device_attribute *attr, char *buf)
{
    psx93XX_t this = dev_get_drvdata(dev);
...
static DEVICE_ATTR(cap_status,0664,sx9310_cap_status_show,NULL);
static DEVICE_ATTR(cap2_status,0664,sx9310_cap2_status_show,NULL);
...
static int initialize(psx93XX_t this)
{
	int ret;
	if (this) {
		pr_info("SX9310 income initialize\n");
		...
		ret = sx9310_global_variable_init(this);
...
static int sx9310_parse_dt(struct sx9310_platform_data *pdata, struct device *dev)
{
	...
	pr_info("[SX9310]:%s -  size of elements %d \n", __func__,pdata->i2c_reg_num);
	...
	/***********************************************************************/
	pr_info("[SX9310]: %s -[%d] parse_dt complete\n", __func__,pdata->irq_gpio);
	return 0;
}
...
static int sx9310_init_platform_hw(struct i2c_client *client)
{
	...
	struct sx9310 *pDevice = NULL;
	struct sx9310_platform_data *pdata = NULL;

	int rc;

	pr_info("[SX9310] : %s init_platform_hw start!",__func__);

	if (this && (pDevice = this->pDevice) && (pdata = pDevice->hw)) {

		pr_info("[SX9310] : %s: pdata->irq_gpio = %d",__func__, pdata->irq_gpio);
		if (gpio_is_valid(pdata->irq_gpio)) {
			rc = gpio_request(pdata->irq_gpio, "sx9310_irq_gpio");
			if (rc < 0) {
				dev_err(this->pdev, "SX9310 Request gpio. Fail![%d]\n", rc);
				return rc;
			}
			rc = gpio_direction_input(pdata->irq_gpio);
			if (rc < 0) {
				dev_err(this->pdev, "SX9310 Set gpio direction. Fail![%d]\n", rc);
				return rc;
			}
			this->irq = client->irq = gpio_to_irq(pdata->irq_gpio);
		}
		else {
			dev_err(this->pdev, "SX9310 Invalid irq gpio num.(init)\n");
		}
	}
	else {
		pr_err("[SX9310] : %s - Do not init platform HW", __func__);
	}
	
	pr_err("[SX9310]: %s - sx9310_irq_debug\n",__func__);
	return rc;
}
...
static int sx9310_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "SX9310 init_platform_hw done!\n");
	
	if (this){
		dev_info(&client->dev, "SX9310 initialize start!!");
		...
		* (1->NIRQ=0, 0->NIRQ=1) */
		...
		pr_info("%s: this->get_nirq_low", __func__, this->get_nirq_low);
		...
		if (pDevice){
			...
			//sysfs_create_group(client, &sx9310_attr_group);
	...
	sx9310_Hardware_Check(this);
	pplatData->exit_platform_hw = sx9310_exit_platform_hw;

	dev_info(&client->dev, "sx9310_probe() Done\n");
 ...
 */
//static int __devexit sx9310_remove(struct i2c_client *client)
...
/*====================================================*/
#else
#define sx9310_suspend		NULL
#define sx9310_resume		NULL
#endif /* CONFIG_PM */
	...
	if (work) {
		...
		while((++counter) < MAX_NUM_STATUS_BITS) { /* counter start from MSB */
			if (((status>>counter) & 0x01) && (this->statusFunc[counter])) {
				dev_info(this->pdev, "SX9310 Function Pointer Found. Calling\n");
...
void sx93XX_suspend(psx93XX_t this)
{
	if (this) {
		disable_irq(this->irq);
		write_register(this,SX9310_CPS_CTRL0_REG,0x20);//make sx9310 in Sleep mode
		dev_info(this->pdev, "SX9310 suspend!\n");
        }
}
void sx93XX_resume(psx93XX_t this)
{
	if (this) {
		enable_irq(this->irq);
		write_register(this,SX9310_CPS_CTRL0_REG,0x27);//Exit from Sleep mode
		dev_info(this->pdev, "SX9310 resume!\n");
	}
}
