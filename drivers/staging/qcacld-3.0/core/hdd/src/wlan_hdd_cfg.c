...
extern char* wcnss_get_fw_adddebugini(void);
...
QDF_STATUS hdd_parse_config_ini(struct hdd_context *hdd_ctx)
{
	...
	ini_add = wcnss_get_fw_adddebugini();
	if(!strcmp(ini_add,"disable") || !strcmp(ini_add,"")) {
		hdd_debug("no extra debug ini");
		goto config_cont;
	} else if(i >= MAX_CFG_INI_ITEMS) {
		hdd_err("Number of items in %d >= %d, do not add debug ini flag" , i , MAX_CFG_INI_ITEMS);
		goto config_cont;
	}

	hdd_debug("add debug ini = %s",ini_add);

	for(pch = strsep(&ini_add, ","); pch != NULL; pch = strsep(&ini_add, ",")) {

		if(!strcmp(pch,"1"))
		       pch_p="qdf_trace_enable_wdi";
		else if(!strcmp(pch,"2"))
		       pch_p="qdf_trace_enable_hdd";
		else if(!strcmp(pch,"3"))
		       pch_p="qdf_trace_enable_sme";
		else if(!strcmp(pch,"4"))
		       pch_p="qdf_trace_enable_pe";
		else if(!strcmp(pch,"5"))
		       pch_p="qdf_trace_enable_wma";
		else if(!strcmp(pch,"6"))
		       pch_p="qdf_trace_enable_sys";
		else if(!strcmp(pch,"7"))
		       pch_p="qdf_trace_enable_qdf";
		else if(!strcmp(pch,"8"))
		       pch_p="qdf_trace_enable_sap";
		else if(!strcmp(pch,"9"))
		       pch_p="qdf_trace_enable_hdd_sap";
		else if(!strcmp(pch,"10"))
		       pch_p="qdf_trace_enable_bmi";
		else if(!strcmp(pch,"11"))
		       pch_p="qdf_trace_enable_cfg";
		else if(!strcmp(pch,"12"))
		       pch_p="qdf_trace_enable_epping";
		else if(!strcmp(pch,"13"))
		       pch_p="qdf_trace_enable_qdf_devices";
		else if(!strcmp(pch,"14"))
		       pch_p="qdf_trace_enable_txrx";
		else if(!strcmp(pch,"15"))
		       pch_p="qdf_trace_enable_dp";
		else if(!strcmp(pch,"16"))
		       pch_p="qdf_trace_enable_htc";
		else if(!strcmp(pch,"17"))
		       pch_p="qdf_trace_enable_hif";
		else if(!strcmp(pch,"18"))
		       pch_p="qdf_trace_enable_hdd_sap_data";
		else if(!strcmp(pch,"19"))
		       pch_p="qdf_trace_enable_hdd_data";
		else if(!strcmp(pch,"20"))
		       pch_p="qdf_trace_enable_wifi_pos";
		else if(!strcmp(pch,"21"))
		       pch_p="qdf_trace_enable_nan";
		else if(!strcmp(pch,"22"))
		       pch_p="qdf_trace_enable_regulatory";
		else if(!strcmp(pch,"23"))
		       pch_p="qdf_trace_enable_cp_stats";
		else {
			hdd_err("unknown value = %s",pch);
		    continue;
		}
		hdd_debug("debug ini item = %s",pch_p);
		cfgIniTable[i].name = pch_p;
		cfgIniTable[i++].value = "511";

		if (i >= MAX_CFG_INI_ITEMS) {
			hdd_err("Number of items in %s > %d , in add debug ini loop",
				WLAN_INI_FILE,
				MAX_CFG_INI_ITEMS);
			break;
		}
